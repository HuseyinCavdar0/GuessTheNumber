...
async def handle_player(ws):
    ...
    is_admin = (remote_ip == "127.0.0.1" or remote_ip == "::1" or remote_ip == "localhost")
    ...
    # Eğer oyuncu global listede yoksa 0 ile başlat
    # Varsa mevcut skorunu korur (ama total 0 başlar, çünkü yeni oyun)
    ...
    print(f"[SERVER] Bağlandı: {name} ({remote_ip}) - Admin: {is_admin}")
        ...
        async for raw in ws:
            ...
            if mtype == "guess":
                ...
                if plus == NUM_DIGITS:
                    ...
                    # Bonus eklendikten sonra REKORU tekrar kontrol et ve kaydet
                    if info["total"] > global_leaderboard.get(info["name"], 0):
                        global_leaderboard[info["name"]] = info["total"]
                        save_leaderboard()
                ...
                request_ip = ws.remote_address[0]
                is_local_req = (request_ip == "127.0.0.1" or request_ip == "::1" or request_ip == "localhost")
                
                if not is_local_req:
                     await ws.send(json.dumps({"type": "error", "msg": "Bu işlemi sadece sunucu bilgisayarı yapabilir."}))
                     continue

                password = str(msg.get("key", ""))
                if password == ADMIN_PASSWORD:
                    global_leaderboard.clear()
                    # Anlık oyuncuları da global listeye 0 puanla geri ekledil
                    for p_info in players.values():
                        global_leaderboard[p_info["name"]] = 0 
                    save_leaderboard()
                    await broadcast(scores_payload())
                    await ws.send(json.dumps({"type": "info", "msg": "Leaderboard başarıyla sıfırlandı."}))
                    print(f"[ADMIN] Localhost kullanıcısı leaderboard'ı sıfırladı.")
                else:
                    await ws.send(json.dumps({"type": "error", "msg": "Yanlış Şifre!"}))
